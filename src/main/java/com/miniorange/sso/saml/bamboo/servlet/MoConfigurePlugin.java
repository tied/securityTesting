...
public class MoConfigurePlugin extends HttpServlet {
    ...
    public MoConfigurePlugin(MoSAMLSettings settings, MoPluginConfigurationsHandler pluginConfigurationsHandler,
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Enumeration headerNames = request.getHeaderNames();
        String key= "", value="";
        Boolean isAdmin = Boolean.FALSE;String username=StringUtils.EMPTY;String pwd=StringUtils.EMPTY;
        while (headerNames.hasMoreElements()) {
            try {
                key = (String) headerNames.nextElement();
                if (StringUtils.equalsIgnoreCase("Authorization", key)) {
        ...
        if(BooleanUtils.toBoolean(isAdmin)){
            ...
            try {
                if (settings.isValidLicense()) {
                    ...
                    showSuccessMessage(response, "SAML settings updated","SAML App Configured successfully.");
                } else {
                    showErrorMessage(response,"Your License has expired"," Please renew the license to use the API Call.");
                }
            } catch (Exception e) {
